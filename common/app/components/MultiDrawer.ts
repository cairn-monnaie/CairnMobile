...
export default class MultiDrawer extends Vue {
    ...
    getDrawerNative(side: string) {
        return this.getDrawer(side).nativeView;
    }
    get backdropNative() {
        return this.backDrop.nativeView as Label;
    }
    ...
    async open(side = null, animationFactor = 1) {
        ...
        try {
            if (animationFactor !== 0) {
                ...
                await this.getDrawerNative(side).animate({
                    translate: {
                        x: 0,
                        y: 0
                    },
    ...
    onDrawerLayoutChange(side) {
        const view = this.getDrawerNative(side);
        const sideData = this.sides[side];
        const optionsInternal = this.optionsInternal[side];
    ...
    onDrawerPan(side, args) {
        ...
        if (args.state === 1) {
            ...
            this.backDrop.opacity = 1 - panProgress;
